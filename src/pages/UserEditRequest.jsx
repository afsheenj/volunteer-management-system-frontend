...
const loadRequest = async () => {
  try {
    ...
    const data = await api.get(`/public/requests/id/${id}`);
...
const handleSubmit = async (e) => {
  ...
  try{
    ...
    if(!form.title || !form.description){
      showWarning("Warning","Title and description required");
      return;
    }

if (!form.minVolunteers || !form.maxVolunteers) {
  showWarning("Volunteers Required", "Please enter min and max volunteers");
  return;
}

    if(Number(form.minVolunteers) > Number(form.maxVolunteers)){
      showWarning("Warning","Min volunteers cannot exceed max volunteers");
      return;
    }
...
navigate(`/user/request/${id}`);
    ...
    console.error(err);
    showError("Error",res.message || "Failed to update request");
  }
  finally{
    setLoading(false);
  }
