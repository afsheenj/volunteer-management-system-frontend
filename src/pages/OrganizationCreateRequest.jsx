...
import { createOrganizationRequest } from "../services/requestServiceOrganization";
...
const handleChange = (e) => {
  ...
  try{
    ...
    if(!form.title || !form.description || !form.landmark || !form.requestType){
      Swal.fire({
  icon: 'warning',
  title: 'Missing Fields',
  text: 'Title, description, Request Type and Landmark are required!',
  confirmButtonText: 'OK'
    ...
    if (!form.minVolunteers || !form.maxVolunteers) {
     Swal.fire({
      ...
      text: 'Please specify the range of volunteers needed.',
    });
    return;
  }

    if(Number(form.minVolunteers) > Number(form.maxVolunteers)){
      Swal.fire({
  icon: 'warning',
  title: 'Invalid Input',
  ...
  confirmButtonText: 'OK'
    ...
    const res =await createOrganizationRequest(user.orgId,payload);
...
const fetchCategories = async () => {
    try {
      const res = await api.get("/enums/categories"); // Your backend endpoint
      // Assuming res is an array of strings: ["OLD_AGE_HOME", "TEACHING", ...]
    ...
    <div className="bg-slate-50 ">
      ...
      <div className="pt-24 px-4 md:px-10 max-w-screen mx-auto min-h-screen bg-indigo-50 flex flex-col gap-4">
        ...
        <h1 className="text-2xl md:text-3xl font-bold text-[#1e40af]">
          Create Request (Organization)
        </h1>
        <p className="text-slate-500">
          Post your service needs and find volunteers for your organization.
        </p>
        ...
        {categories.map((cat) => (
          <option key={cat} value={cat}>
            {/* 4. Format the Enum (e.g., OLD_AGE_HOME -> Old Age Home) */}
                    ...
                    >
                      <option value="">Select</option>
                      {
                        citiesList.map((value)=>{
                          return <option value={value.name}  key={value.id || value.name}>{value.name}</option>
                        })
                      }
                    </select>
            ...
            {/* <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div className="flex flex-col gap-2">
                ...
                >
                  ...
                  <option>Unregistered Community Group</option>
                  <option>Student / College Group</option>
                  <option>Social Service Group</option>
                  <option>Corporate Volunteer Team</option>
              ...
              <div className="py-4 flex flex-col gap-2">
                <label className="flex items-center gap-2  text-slate-700">
                  <input type="checkbox" />
                  Accept individual volunteers (general public)
                </label>
                <label className="flex items-center gap-2 text-slate-700">
                  <input type="checkbox" />
                  Allow volunteers to view my contact
                </label>
              </div>
            </div> */}
...
export default OrganizationCreateRequest;
